...
class HotPhotoWall extends Component {
	...
	itemOnClick(markerArr) {
        ...
        if (obj.panoID == constants.c_StationInfo.panoID) { //在当前站点的文物
            constants.c_markerInfoArr.map((objs,idx) =>{
	...
	createTable() {
		...
		this.pageArr.map((itemArr, idxs) => {
			table.push(
				<div className="item">
					<ul>
						{itemArr.map((items, idx) => {
							return (
								<Exhiblistbox
									key={`HotPhotoWall${idx}`}
									active={false}
									getPhotoWall={this.itemOnClick.bind(this)}
									markerID={items.markerID}
									name={items.name}
									sanwei={items.have3D}
									imgUrl={`${constants.sw_getService
										.getmusServerURL()
										.split('/S')[0]}/${items.filePath}`}
								/>
							);
						})}
					</ul>
