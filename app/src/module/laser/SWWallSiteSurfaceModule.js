...
class SWWallSiteSurfaceModule {
    ...
    createGroundMash() {
        ...
        let texture = new THREE.TextureLoader().load(external.siteFace_icon);
        ...
        let geometry = new THREE.PlaneGeometry(76, 76, 1, 1);
    ...
    groundFaceMove(obj) {
        ...
        if (this.isgroundFaceJumpBoo != "") {
            ...
            this.groundFaceMash.position.copy(obj.point);

            c_arrowArr.forEach((sitedata) => {

                sitedata.setMeshOpacity(this.isgroundFaceJumpBoo);
            ...
            this.textDiv.style.left = (labelPos.x - 35) + "px";

            this.textDiv.style.top = (labelPos.y - 40) + "px";

            this.textDiv.innerHTML = "前进 " + this.xround(getProbeSurfaceDistance(obj) * 0.6, 2) + " 米";
    ...
    wallFaceMove(obj) {
        ...
        let jumpPano = getJudgeOrZoom(obj, 2);

        this.isWallFaceJumpBoo = (jumpPano != "");
        ...
        c_arrowArr.forEach((sitedata, idx) => {

            sitedata.setMeshOpacity();

        });

        let fov = camera.fov / c_Maxfov; //相机缩放比例

        let dis = (obj.distance - 14) * 0.0015; //0.0015是每一米对应缩放比例

        dis = dis < 0.03 ? 0.03 : dis; //0.05是最小缩放比例

        dis = dis > 1 ? 1 : dis; //2是最大缩放比例
...
export default SWWallSiteSurfaceModule;